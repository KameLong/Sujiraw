...
export function useDiagramViewHook(companyID: number, routeID: number) {
    ...
    useEffect(() => {
        loadCompany(companyID, routeID).then(async (company) => {
            ...
            const downTrips = route.downTrips.map(item => {
                return {
                    ...item, stopTimes: item.times.map(item => {
...
export function useDiagramViewHook2(diagramData: DiagramDataDTO) {
    const [routeStations, setRouteStations] = useState<DiagramStation[]>([]);
    const [downLines, setDownLines] = useState<DiagramLine[]>([]);
    const [upLines, setUpLines] = useState<DiagramLine[]>([]);

    const stations = diagramData.stations;
    const trainTypes = diagramData.trainTypes;

    const route=diagramData.route;
    console.log(route,diagramData);
    console.log(route.routeStations.length)
    ...
    useEffect(() => {
        ...
        const rs: DiagramStation[] = [];
        rs.push({
            ...route.routeStations[0],
            stationTime: 0,
            station: stations[route.routeStations[0].stationID]});
        let nowStationTime = 0;
        for (let i = 1; i < route.routeStations.length; i++) {
            const stationID = route.routeStations[i].stationID;
            const station = stations[stationID];
    ...
    return {routeStations, downLines, upLines};
}